...
PYEOF
    ...
    result=$(osascript - "$summary" "$description" "$calendar" "$recurrence" \
        ...
        "$all_day" "$iso_date" "$location" <<'APPLESCRIPT'
on run argv
        ...
        tell calendar calName
            if isAllDay then
                if evtLocation is not "" then
                    set newEvent to make new event with properties {summary:evtSummary, start date:startDate, end date:startDate, allday event:true, description:evtDescription, location:evtLocation}
                else
...
APPLESCRIPT
  ...
  help|*)
    ...
    echo "  location       Event location (address or place name)"
